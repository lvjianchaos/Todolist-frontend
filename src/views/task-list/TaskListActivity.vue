...
async function loadPage() {
  ...
  loading.value = true
  try {
    const params: { page: number; size: number; listId?: number } = { page: page.value, size: size.value }
    if (listId.value !== null) params.listId = listId.value
...
// when listId changes (route param), reset paging and reload
watch(listId, () => {
  page.value = 1
