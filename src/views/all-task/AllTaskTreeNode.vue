...
import { taskTreeContextKey, type DateField, type TaskMenuCommand } from '@/views/task-list/taskTreeContext'

defineOptions({ name: 'AllTaskTreeNode' })
...
<template>
  <div class="task-node">
    <el-dropdown class="task-menu" trigger="contextmenu" placement="bottom-start" @command="onTaskMenuCommand">
      <div class="task" :class="{ 'is-done': ctx.statusDone(task) }" @click="() => ctx.openDrawerFromBlank(task)">
        <div class="col-title">
      ...
      <template v-else>
        <div class="task-list">
          <div v-for="child in childrenSortedForView" :key="child.id" class="task-wrap">
            <AllTaskTreeNode :task="child" :depth="depth + 1" />
          </div>
        </div>
